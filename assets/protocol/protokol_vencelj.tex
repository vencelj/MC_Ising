...
\usepackage{booktabs} % 
\usepackage[colorlinks=true]{hyperref} % functional links
...
\begin{document}
	...
	\paragraph{Metropolisův algoritmus} je založen na generování náhodného čísla představující pravděpodobnost $p(\sigma_i|T)$ (přepsáno pro stavy $a$ a $b$ $p_{a \rightarrow b}$). Pokud platí $\Delta E(\sigma_i) < 0$, proveď změnu stavu $a \rightarrow b$ s pravděpodobností $p_{a \rightarrow b} = 1$. V opačném případě proveď změnu stavu, pokud je vygenerovaná pravděpodobnost menší než partiční funkce $p_{a \rightarrow b} < \exp(-E(\sigma)/\mathrm{\mathbf{k_B}}T)$ \cite{FitzpatrickIsing}.
	...
	Data jsem proložil závislostí $M(T) = \alpha(T - T_\mathrm{c})^\beta$, ze které jsem odečetl kritickou teplotu $T_\mathrm{c} = 4,462$ a kritickým exponentem $\beta = 0,273$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{fitt.png}
		\caption{Získání kritických konstant}
		\label{fitt}
	\end{figure}
	...
	Provedl jsem 5 simulací s velikostmi mřížky 10, 20, 30, 40 a 50. Velikost mřížky má vliv na přesnost odečítání kritické teploty. Pro znázornění jsem vypočetl tepelnou kapacitu $C = \dv{E}{T}$. 
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{tep_cap.png}
		\caption{Tepelná kapacita}
		\label{tep_cap}
	\end{figure}
